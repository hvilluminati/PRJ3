...
function getDrank(cookie)
{
    ...
    {
        sid = (db.exists("/myarray[" + i + "]") ? db.getData("/myarray[" + i + "]/sid").trim() : "nop");
        ssid = (sid != cookie ? db.getData("/myarray[" + i + "]/ssid").trim() : "nop");
...
fs.watch(__dirname + '/qr_read/most_recent_qr', (event, filename) => {
    if (filename && event ==='change' && !fsTimeout) {
...
app.get('/cookie', function(req, res) {
    ...
    user = {
        sid:    (typeof req.cookies["connect.sid"] !== undefined ? req.cookies["connect.sid"] : "nop"),
        ssid:   req.sessionID,
